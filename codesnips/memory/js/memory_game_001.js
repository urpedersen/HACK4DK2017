...
function initialize() {
	...
	for ( var x=0 ; x<rows ; x++ ) {
		for ( var y=0 ; y<cols ; y++ ) {
			...
			img[i].isShown=false;	// True if the player have turned this card
			img[i].isFound=false;	// True if player have identified this cards
...
function mouse_click(e) {
	...
	var x=Math.floor( e.x/width*rows ) ;
	var y=Math.floor( e.y/height*cols ) ;
	var i = index(x,y);
	console.log('Mouse clicked: e.x=' + e.x + ' e.y=' + e.y + ' x=' + x + ' y=' +y +' img[i].isShown= ' + img[i].isShown);
